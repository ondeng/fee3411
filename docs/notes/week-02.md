...
**NB:** The first part builds the general framework and applies it to electrical networks. The second part applies the *same* framework to mechanical systems, to gears, and to the electromechanical system that combines both: the d.c. servomotor. In the tutorial, the servomotor and linearisation are elaborated through practice problems.
...
Three routes lead to a model of this form: first-principles physical laws, identification from measured input–output data, or a combination of the two. This week is entirely about the first route, which is the one used throughout FEE3411.
...
<figcaption><strong>Figure 1.</strong> Series <span class="arithmatex">\(RLC\)</span> network with source voltage <span class="arithmatex">\(v_i(t)\)</span> and the capacitor voltage <span class="arithmatex">\(v_o(t)\)</span> taken as output.</figcaption>
</figure>
...
<figcaption><strong>Figure 2.</strong> Mass–spring–damper system and its free-body diagram: the spring and damper forces oppose the applied force <span class="arithmatex">\(f(t)\)</span>.</figcaption>
</figure>
...
Rotational systems follow the same pattern with torque $T(t)$, angle $\theta(t)$, moment of inertia $J$, torsional spring constant $K$ and viscous friction coefficient $b$: 
...
<figcaption><strong>Figure 3.</strong> Rotational system: a torsional spring (represented here by the coiled support) and viscous friction <span class="arithmatex">\(b\)</span> restrain an inertia <span class="arithmatex">\(J\)</span> driven by an applied torque <span class="arithmatex">\(T(t)\)</span>.</figcaption>
</figure>
...
A gear train couples two shafts turning at different speeds and torques. Two meshed gears with $N_1$ and $N_2$ teeth turn through angles related by $N_1\theta_1=N_2\theta_2$ (the teeth that mesh must match), so with $n\triangleq\theta_2/\theta_1=N_1/N_2$ the speeds are in ratio $n$ while, since power is conserved across an ideal (lossless) gear pair, the torques are in the inverse ratio $T_2/T_1=1/n$.

<figure id="fig:gears" data-latex-placement="htbp">
<img src="../svg/week-02-notes/fig05.svg" />
<figcaption><strong>Figure 4.</strong> A gear train: the motor drives the small gear (<span class="arithmatex">\(N_1\)</span> teeth), meshed with the load’s larger gear (<span class="arithmatex">\(N_2\)</span> teeth), so the load turns more slowly and with more torque than the motor.</figcaption>
</figure>

Suppose a motor of inertia $J_m$ and friction $b_m$ drives, through a gear train of ratio $n=\theta_L/\theta_m$, a load of inertia $J_L$ and friction $b_L$. Let $T_1$ be the reaction torque the gear train exerts back on the motor shaft and $T_2$ the torque it delivers to the load, so that each shaft obeys its own equation of motion separately, 

$$J_m\ddot\theta_m + b_m\dot\theta_m = T_m - T_1, \qquad
  J_L\ddot\theta_L + b_L\dot\theta_L = T_2 ,$$

 coupled only through the gear relations $\theta_L=n\theta_m$ and (lossless mesh, as in §[5](#sec:gears) above) $T_2=T_1/n$. Substituting $\theta_L=n\theta_m$ into the load equation and using $T_2=T_1/n$ gives the reaction torque purely in terms of the motor’s own motion, 

$$T_1 = n^2\big(J_L\ddot\theta_m+b_L\dot\theta_m\big) ,$$

 and substituting this $T_1$ into the motor equation eliminates the load variables entirely, leaving a single equation in $\theta_m$ alone: 
...
This is the electromechanical system mentioned in Week 1: it combines the electrical modelling of §[2](#sec:electrical) with the rotational modelling of §[4](#sec:mech-rot), coupled by the motor’s two electromechanical constants.

<figure id="fig:dcmotor" data-latex-placement="htbp">
<img src="../svg/week-02-notes/fig06.svg" />
<figcaption><strong>Figure 5.</strong> Armature-controlled d.c. servomotor: the armature circuit (<span class="arithmatex">\(R_a\)</span>, <span class="arithmatex">\(L_a\)</span>) drives the rotor (inertia <span class="arithmatex">\(J_m\)</span>, friction <span class="arithmatex">\(b\)</span>) against its own back e.m.f. <span class="arithmatex">\(e_b\)</span>; the field is separately excited and held constant.</figcaption>
</figure>
...
Many physical systems are nonlinear. Their equations may contain terms such as 

$$\sin\theta,\qquad x^2,\qquad xy,\qquad e^x,$$

 which make analysis more difficult than for linear systems. The key engineering idea is that a smooth nonlinear function often looks almost like a straight line when viewed over a sufficiently small region. Therefore, near a chosen operating point, we can replace the nonlinear system by a linear approximation.

A key tool in performing the linearization is the Taylor series. A Taylor series is a systematic way of replacing a function by a polynomial that behaves like the function near a chosen point.

Suppose a function $f(x)$ is known near $x=a$. Its Taylor expansion about $a$ is
...
The **first-order Taylor approximation** keeps terms only up to $(x-a)^1$:

$$\begin{equation}
f(x)\approx f(a)+f'(a)(x-a)
\end{equation}$$

This matches the function’s **value and slope** at $x=a$. Geometrically, it is the tangent line.

Therefore, for a scalar function $y=f(x),$ its first-order Taylor expansion about an operating point $x=x_0$ is 

$$f(x)\approx f(x_0)+f'(x_0)(x-x_0).$$

 If $y_0=f(x_0)$ and we define small deviations 
...
/// admonition | Worked example 2.6 — Linearization
    type: example

**Example: $y=x^2$ about $x_0=3$.** We have 
...
$$y\approx 9+6(0.1)=9.6,$$

 whereas the exact value is 

$$3.1^2=9.61.$$

 Close to the operating point, the tangent-line model is very good.

///

#### 7.2. The pendulum: exact nonlinear model

Consider the pendulum shown in Figure [6](#fig:pendulum). There are two equivalent ways to derive the motion equation: using tangential forces or using torques.

<figure id="fig:pendulum" data-latex-placement="htbp">
<img src="../svg/week-02-notes/fig07.svg" />
<figcaption><strong>Figure 6.</strong> The simple pendulum: rigid massless rod of length <span class="arithmatex">\(l\)</span>, bob of mass <span class="arithmatex">\(m\)</span>, viscous friction coefficient <span class="arithmatex">\(k\)</span> (not shown) resisting the swing.</figcaption>
</figure>

##### Tangential-force description { #tangential-force-description }

The bob moves along a circular arc. Its tangential acceleration is $a_t=l\ddot\theta.$ Newton’s second law in the tangential direction is therefore $F_t=ma_t=ml\ddot\theta.$

The tangential component of gravity is $-mg\sin\theta,$ where the minus sign indicates that gravity acts to restore the pendulum toward $\theta=0$.

If viscous damping is included, with damping coefficient $k$, then $F_d=-kv.$ Since the bob’s tangential speed is $v=l\dot\theta,$ the damping force is $F_d=-kl\dot\theta.$ Thus 
...
For rotational motion, $\tau=J\alpha,$ where $J$ is the moment of inertia and $\alpha=\ddot\theta$.

For a point mass at radius $l$, $J=ml^2.$ The gravitational torque is 

$$\tau_g=(-mg\sin\theta)l=-mgl\sin\theta,$$

 and the damping torque is 

$$\tau_d=(-kl\dot\theta)l=-kl^2\dot\theta.$$

 Therefore 

$$ml^2\ddot\theta=-mgl\sin\theta-kl^2\dot\theta,$$
...
**Why the two forms look different.** The tangential-force equation contains $ml\ddot\theta$, while the torque equation contains $ml^2\ddot\theta$. The torque equation is simply the force equation multiplied by the moment arm $l$. Both describe exactly the same physics.

///

##### Small-angle linearization { #small-angle-linearization }

The Taylor series of $\sin\theta$ about $\theta=0$ is 

$$\sin\theta
=
\theta-\frac{\theta^3}{3!}+\frac{\theta^5}{5!}-\cdots.$$

 For sufficiently small $\theta$ (measured in radians), the higher-order terms are much smaller than the first term, so $\sin\theta\approx\theta.$

The same result follows from first-order Taylor linearization. Let 

$$f(\theta)=\sin\theta.$$

 Around the equilibrium $\theta_0=0$, 

$$f(\theta)\approx f(0)+f'(0)(\theta-0).$$
...
$$\sin\theta\approx\theta.$$
 ...
 This approximation is applied to the damped pendulum in the example below.

/// admonition | Worked example 2.6b — linearizing the pendulum
    type: example
...
A linearization is only valid near the point it was taken about. The two equations in Worked example 2.6b look almost identical but describe opposite behaviour, because they linearize about a stable and an unstable equilibrium respectively. Quoting a linearized model without stating which operating point it belongs to is a meaningless answer.

///

#### 7.3. Linearization of a nonlinear dynamic system

For a *dynamic* system $\dot x=f(x,u)$, linearization is done about an equilibrium point $(x_0,u_0)$ satisfying $f(x_0,u_0)=0$ — a point where the system, once there, stays there with no input change — using the same first-order Taylor idea on each variable. Expanding $\dot x=f(x,u)$ to first order about $(x_0,u_0)$ and dropping the (zero) constant term $f(x_0,u_0)$ gives the linearized state equation 

$$\Delta\dot x \;\approx\; \left.\frac{\partial f}{\partial x}\right|_{x_0,u_0}\!\!\Delta x
  \;+\; \left.\frac{\partial f}{\partial u}\right|_{x_0,u_0}\!\!\Delta u ,
  \qquad \Delta x=x-x_0,\quad \Delta u=u-u_0 ,$$

 i.e. each partial derivative is just a constant (the Jacobian evaluated at the operating point, a single number for a scalar system), so the result is linear in the deviations $\Delta x,\Delta u$ even though $f$ itself need not be.
