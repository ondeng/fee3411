...
%   [x] 4. Hydraulic power elements (pump-motor, valve, cylinder)
%   [x] 5. Pneumatic power elements (bellows, flapper-nozzle, relay, diaphragm)
%
...
\bigskip
\begin{readingbox}
A \emph{longer} route through Part~B of the Week~1 notes --- all five
components: error detectors, actuators, sensors, and hydraulic and pneumatic
power elements. The main notes (\texttt{week-01-notes.pdf}) state each
component's governing equation and say where it comes from; the abridged
version (\texttt{week-01-notes-partB-brief.pdf}) states it in words alone.
This document does the opposite: it starts from the physics and builds each
...
ed., \S4.3--4.6, pp.~84--119. Nagrath states the results this document
derives.
\end{readingbox}

\bigskip
\begin{outcomes}
\item Derive the governing equation of every component in Part~B --- error
      detectors, actuators, sensors, and hydraulic and pneumatic power
      elements --- from a physical law, rather than quoting it.
\item Recognise the transfer function $K/[s(\tau s+1)]$ wherever it appears
      (d.c.\ and a.c.\ servomotors, hydraulic pump--motor and cylinder), and
      say in each case \emph{why} the free integrator is there.
\item Explain three recurring reasons a manufacturer builds a component the
      way it does: high rotor resistance in an a.c.\ servomotor, a spring in
      a pneumatic actuator but not a hydraulic one, and rate feedback added
      purely to change damping.
\item Quantify, rather than merely state, the approximations behind a model
      --- a linearisation error, a neglected leakage term --- and know when
      each stops being safe to make.
\end{outcomes}
...
Where the load is large, hydraulics win. A hydraulic motor delivers far more
power for its size than an electric motor, and hydraulic components are fast
and very stiff under load --- push against a hydraulic ram and it does not
yield the way a spring does. Set against that: leaks, sealing against
contaminated oil, noise, sluggishness when the oil is cold and thick, and
lines that are far less flexible than a cable. Typical uses are power
steering and brakes, ship steering gear, large machine tools, aircraft flight
controls and excavators. Nagrath \S4.5, p.~104.

Hydraulic output devices split by the motion they produce: \textbf{rotary
output} from a \emph{hydraulic motor} (what the syllabus calls a ``rotary
actuator''), and \textbf{translational output} from a \emph{hydraulic linear
actuator} --- a cylinder or ram. Both stories below end at the same place:
\begin{enumerate}[leftmargin=2em,itemsep=1pt]
\item Write a flow balance and a force (or torque) balance for the device
      (\S\ref{sss:swashplate}, \S\ref{sss:cylinder}).
\item Eliminate the pressure between them.
\item Discover, again, the shape $K/[s(\tau s+1)]$ that has now appeared for
      the d.c.\ servomotor \eqref{eq:dcm}, the a.c.\ servomotor \eqref{eq:acm},
      and every hydraulic actuator in this section --- and see \emph{why} it
      keeps appearing (the pitfall at the end of \S\ref{sss:cylinder}).
\end{enumerate}
...
\textbf{fixed-stroke motor}. Both are axial-piston machines: several pistons,
arranged in a circle in a rotating cylinder block, bear on a stationary
\emph{swash plate} through shoes. Figure~\ref{fig:swashplate} shows the idea.
With the swash plate perpendicular to the shaft (the dashed ``neutral''
position) a piston's distance to the plate does not change as the block
turns, so no piston reciprocates and no oil is pumped. Tilt the plate through
the \textbf{stroke angle} $x$ and that distance now varies once per
revolution: as a piston's bore turns past the plate, it is first pushed in
and then drawn back out, once per turn, so it pumps. Reversing the tilt
reverses which side pumps and which side returns, and hence reverses the
motor. One mechanical variable, the stroke angle $x$, controls everything ---
flow rate, direction, motor speed.

\begin{figure}[H]
\centering
\begin{tikzpicture}[font=\small, x=1cm, y=1cm]
% ---- shaft ----
\draw[thick] (-1.7,0) -- (0,0);
\node[font=\scriptsize, below] at (-1.05,-0.08) {shaft};
\draw[-{Latex[length=1.6mm]}, gray] (-1.55,0.55) arc (150:30:0.55);
\node[gray, font=\scriptsize, above] at (-1.05,0.78) {rotation};
% ---- rotating cylinder block ----
\draw[thick] (0,-1.55) rectangle (1.6,1.55);
\node[font=\scriptsize, align=center] at (0.8,0) {rotating\\block};
% top bore + piston + rod + shoe (retracted: plate close on this side)
\draw[thick] (0.25,0.95) rectangle (1.45,1.35);
\draw[thick, fill=fnavy!25] (0.30,1.00) -- (1.00,1.00) -- (1.00,1.30) -- (0.30,1.30) -- cycle;
\draw[very thick] (1.00,1.15) -- (2.22,1.03);
\draw[fill] (2.22,1.03) circle (2.3pt);
% bottom bore + piston + rod + shoe (extended: plate far on this side)
\draw[thick] (0.25,-1.35) rectangle (1.45,-0.95);
\draw[thick, fill=fnavy!25] (0.30,-1.30) -- (1.30,-1.30) -- (1.30,-1.00) -- (0.30,-1.00) -- cycle;
\draw[very thick] (1.30,-1.15) -- (2.98,-1.03);
\draw[fill] (2.98,-1.03) circle (2.3pt);
% ---- swash plate: neutral (dashed) and tilted (solid), pivoted at (2.6,0) ----
\draw[gray, dashed] (2.6,-1.85) -- (2.6,1.85);
\node[gray, font=\scriptsize, above] at (2.6,1.9) {neutral};
\begin{scope}
  \draw[thick, faccent, line width=2.4pt, rotate around={20:(2.6,0)}] (2.6,-1.85) -- (2.6,1.85);
\end{scope}
\draw[thick] ($(2.6,0)+(90:0.4)$) arc (90:110:0.4);
\node[font=\scriptsize] at ($(2.6,0)+(101:0.65)$) {$x$};
\node[font=\scriptsize, faccent, align=left] at (3.55,1.5) {swash\\plate};
% ---- ports ----
\draw[sig, faccent] (0.8,-1.55) -- (0.8,-2.35);
\node[faccent, font=\scriptsize, below, align=center] at (0.8,-2.4)
   {delivery $q_{p}=K_{p}x$\\(to motor)};
\draw[sig] (0.35,1.55) -- (0.35,2.15);
\node[font=\scriptsize, above, align=center] at (0.35,2.2) {return\\(from motor)};
\end{tikzpicture}
\caption{Axial-piston pump (schematic, one representative piston pair). As the
block rotates, a piston whose bore is on the side where the tilted plate sits
\emph{closer} is pushed in; half a turn later, on the side where the plate is
\emph{farther}, the same piston is drawn back out. That reciprocation is the
pumping action, and its amplitude --- hence the flow --- is set by the stroke
angle $x$. A fixed-stroke motor runs the same machine in reverse: fixed $x$,
oil in, shaft out. Cf.\ Nagrath \& Gopal \S4.5, p.~104.}
\label{fig:swashplate}
\end{figure}

\subsubsection{From two balances to one transfer function}
\label{sss:swashplate}

Five relations, none of them more than a statement that something is
proportional to something else, are all the model needs.

\textbf{Flow.} The pump delivers oil at a rate set by the stroke angle; the
motor swallows oil at a rate set by its own speed; and real oil is not
perfectly contained, so a fraction leaks past the pistons in proportion to the
pressure drop across them, and a further fraction goes into compressing the
oil itself, in proportion to how fast the pressure is changing:
\[
  q_{p}=K_{p}x, \qquad q_{m}=K_{m}\dot\theta, \qquad
  q_{\ell}=K_{\ell}p, \qquad q_{c}=K_{c}\dot p .
\]
Continuity says the pump's output has to go \emph{somewhere}:
\begin{equation}
  q_{p}=q_{m}+q_{\ell}+q_{c}.
  \label{eq:hydcontinuity}
\end{equation}

\textbf{Torque.} The motor develops a torque in proportion to the pressure
across it, and that torque drives the load's inertia and friction:
\begin{equation}
  T_{m}=K_{T}p=J\ddot\theta+f\dot\theta .
  \label{eq:hydtorque}
\end{equation}

Two balances, one unknown pressure $p$ to eliminate between them. Before doing
the algebra, drop the compressibility term: $K_{c}\dot p$ is small compared
with $K_{m}\dot\theta$ because oil is nearly incompressible, and keeping it
would add a third pole to what is about to become a second-order system ---
correct, but a complication this course does not need. (If it mattered, it
would show up as a very fast additional lag; \eqref{eq:hydcontinuity} shows
exactly where to put it back.) With $q_{c}$ dropped, solve
\eqref{eq:hydtorque} for $p=(J\ddot\theta+f\dot\theta)/K_{T}$ and substitute
into \eqref{eq:hydcontinuity}:
\[
...
\begin{equation}
  ...
  \label{eq:hyd}
\end{equation}
The same shape as \eqref{eq:dcm} and \eqref{eq:acm} --- and for the same
reason: $x$ commands a \emph{flow}, flow drives a \emph{speed}, and the
output of interest is the \emph{position} that speed integrates to.

\begin{keyidea}
$K_{T}$ and $K_{m}$ look like two independent constants, but they are not, and
the argument is exactly the one that gave $K_{t}=K_{b}$ for the d.c.\ motor in
\S\ref{sss:torqueconst}. An ideal (lossless) hydraulic motor converts flow
power to mechanical power with nothing wasted: $p\,q_{m}=T_{m}\dot\theta$.
Substitute $q_{m}=K_{m}\dot\theta$ and $T_{m}=K_{T}p$:
\[
  p\,K_{m}\dot\theta = K_{T}p\,\dot\theta \quad\Longrightarrow\quad K_{T}=K_{m}.
\]
A hydraulic motor's torque constant and its volumetric displacement are the
same number, for the same reason a d.c.\ motor's torque constant and
back-e.m.f.\ constant are the same number: power balance, not coincidence.
With $K_{T}=K_{m}$, \eqref{eq:hyd} simplifies to
$K=K_{p}K_{m}/(K_{m}^{2}+K_{\ell}f)$ and $\tau=K_{\ell}J/(K_{m}^{2}+K_{\ell}f)$.
\end{keyidea}
...
throttle the flow with a \textbf{spool valve}, Figure~\ref{fig:spool}. The
spool is far lighter than a pump's stroke mechanism, so its time constants
are much smaller and the whole system responds much faster --- the cost of
that speed is that valve flow is genuinely nonlinear: flow through a
sharp-edged orifice goes as $\sqrt{\Delta p}$, not as $\Delta p$. Writing
\begin{equation}
  q = K_{1}x - K_{2}p
  \label{eq:valve}
\end{equation}
is therefore a \emph{linearisation} about the neutral operating point ---
valid for small spool displacements, invalid for large ones --- built by the
same first-order Taylor-expansion technique used for the a.c.\ servomotor's
torque--speed surface in \S\ref{sss:highR}.

\statedin{Week~2 works through the general linearisation technique in full;
Worked example 4.1 below uses only its result, \eqref{eq:valve}.}

A \textbf{three-way} valve has one supply port, one sump port and one service
port; a \textbf{four-way} valve has two service ports and can drive a
double-acting cylinder in both directions, which is what
Figure~\ref{fig:spool} shows. At neutral, $x=0$, both service ports are
blocked. Move the spool one way and the supply connects to one side of the
piston while the other side drains to sump; move it the other way and the
connections swap.
...
Three equations describe Figure~\ref{fig:spool} completely: the linearised
valve relation \eqref{eq:valve}; continuity, with the flow into the cylinder
equal to the piston's swept-volume rate, $q=A\dot y$; and the force balance on
the load, driven by the pressure difference acting on the piston area $A$,
\[
  Ap = M\ddot y + f\dot y .
\]
Eliminate $p$ first (solve the force balance for it) and substitute into the
valve relation to eliminate $q$ as well:
\[
  A\dot y = K_{1}x - K_{2}\left(\frac{M\ddot y+f\dot y}{A}\right).
\]
Multiply through by $A$, collect the $\ddot y,\dot y$ terms, and take Laplace
transforms:
\[
...
\begin{equation}
  ...
  \label{eq:cyl}
\end{equation}
Again the same shape. Leakage past the piston seal ($K_{2}$) is usually small
enough that $K_{2}f\ll A^{2}$, and then \eqref{eq:cyl} collapses to the tidier
$K\approx K_{1}/A$, $\tau\approx MK_{2}/A^{2}$ quoted in the main notes --- but
``usually small enough'' is a claim to check, not to assume, which is exactly
what Worked example 4.1 does.

\begin{pitfall}
Note where the $1/s$ comes from. It is \emph{not} an approximation, and it is
not the load's inertia --- inertia is what supplies the $(\tau s+1)$ lag, not
the integrator. Flow into a cylinder sets the piston's \emph{velocity}, so
position is the integral of that velocity. \textbf{Any} actuator whose input
commands a rate --- valve opening $\to$ velocity, armature voltage $\to$
speed, pump stroke $\to$ motor speed, field current $\to$ (\S\ref{sss:field})
--- carries a free integrator whenever the output of interest is position.
Once you see it this way, you stop needing to re-derive it for every new
actuator: check what the input commands, and the $1/s$ either has to be there
or it does not.
\end{pitfall}

\begin{workedex}[breakable, title={Worked example 4.1 --- how good is $K\approx K_{1}/A$?}]
A cylinder has piston area $A=\SI{5e-3}{\metre\squared}$, valve gain
$K_{1}=\SI{0.025}{\metre\squared\per\second}$, combined leakage and
compressibility coefficient
$K_{2}=\SI{8.33e-9}{\metre\cubed\per\second\per\pascal}$, load mass
$M=\SI{20}{\kilo\gram}$ and friction $f=\SI{150}{\newton\second\per\metre}$.

\emph{Exact values, from \eqref{eq:cyl}:}
\[
  A^{2}=\SI{2.5e-5}{\metre\tothe{4}}, \qquad
  K_{2}f=\SI{1.25e-6}{\metre\cubed\per\pascal}
  \;\Longrightarrow\; \frac{K_{2}f}{A^{2}}=5.0\%.
\]
\[
  K=\frac{AK_{1}}{A^{2}+K_{2}f}=\SI{4.76}{\per\second},
  \qquad
  \tau=\frac{MK_{2}}{A^{2}+K_{2}f}=\SI{6.35}{\milli\second}.
\]
\emph{Approximate value:} $K\approx K_{1}/A=\SI{5.00}{\per\second}$ --- 5.0\%
high, the same 5.0\% by which $K_{2}f$ falls short of $A^{2}$ (the two
percentages are the same number: $K/K_{\text{approx}}=A^{2}/(A^{2}+K_{2}f)$).
For this seal, the shortcut is good to one part in twenty --- fine for a
first design pass, worth checking against the exact formula before a report
goes out. Note also how fast this actuator is: $\tau\approx\SI{6}{\milli
\second}$, two orders of magnitude faster than the $\tau_{m}=\SI{0.2}{\second}$
motor of Worked example 3.1 --- exactly the speed advantage valve control was
built to buy.
\end{workedex}

% ====================================================================
\section{Pneumatic power elements}
\label{sec:pneumatics}

Pneumatic systems use air instead of oil. Air is non-inflammable,
intrinsically safe, and its viscosity is negligible and barely changes with
temperature the way hydraulic oil's does. The price is that air is
\textbf{compressible}, so pneumatic systems carry a substantial compressibility
flow and are characterised by \textbf{longer time delays} --- the reason they
dominate in \emph{process} control, where the plant itself is slow, and are
rare wherever speed matters. Nagrath \S4.6, p.~116.

Four devices make up a pneumatic control chain, each converting one signal
into the next: a mechanical input becomes a pressure (bellows or
flapper--nozzle), a small pressure signal becomes a large one (the relay),
and a pressure becomes a mechanical output again (the diaphragm actuator).
Three of the four are pure algebra; only the last needs a differential
equation.
...
A hollow chamber with thin corrugated side walls and flat end faces behaves as
a spring. A pressure difference $\Delta P$ across the end faces (area $A$)
produces a separating force $(\Delta P)A$; the corrugated walls produce a
restoring force $K\Delta x$ proportional to the displacement. At equilibrium
these balance, $(\Delta P)A = K\Delta x$, so
\begin{equation}
  \frac{\Delta X(s)}{\Delta P(s)}=\frac{A}{K}.
  \label{eq:bellows}
\end{equation}
A pure gain, no dynamics at all --- pressure in, displacement out, instantly.
(A real bellows does have some mass and some air compressibility inside it,
which would add a fast second-order lag; at the frequencies this course works
with, that lag is negligible next to everything else in the loop, so
\eqref{eq:bellows} is taken as exact.)

% --------------------------------------------------------------------
\subsection{The flapper--nozzle valve}
\label{sss:flappergain}

The key pneumatic \emph{sensing} element, and the pneumatic analogue of the
potentiometer or the synchro: it turns a small mechanical displacement into an
electrical-strength --- here, pneumatic-strength --- signal. Air at constant
supply pressure $P_{s}$ passes through a fixed \emph{orifice} and out of a
\emph{nozzle}. A pivoted \emph{flapper} sits a distance $e$ in front of the
nozzle, Figure~\ref{fig:flapper}. Move the flapper closer and the escape
route narrows, so back pressure builds up in the chamber between orifice and
nozzle and $P_{b}$ rises towards $P_{s}$; move it away and the restriction
...
The lever arms $a$ (pivot to nozzle) and $b$ (pivot to input point) fix the
gap in terms of the input displacement, $e=[a/(a+b)]x$: it is the same
lever-arm bookkeeping used for any pivoted linkage, nothing pneumatic about
it yet. What \emph{is} pneumatic is the characteristic of $P_{b}$ against
$e$: it is strongly nonlinear over its full range (as $e\to 0$ the nozzle
seals and $P_{b}\to P_{s}$; as $e$ grows large, $P_{b}$ saturates at the
value set by the orifice alone), but it has one steep, nearly straight region
in between, and that is the region the device is operated in. Writing $K$ for
the local slope $\mathrm{d}P_{b}/\mathrm{d}e$ there (negative: closing the gap
raises the pressure) and combining it with the lever-arm relation,
\begin{equation}
  \frac{\Delta P_{b}(s)}{\Delta X(s)}=\left(\frac{a}{a+b}\right)K,
  \qquad K<0.
  \label{eq:flapper}
\end{equation}
$K$ is large --- typically a fraction of a millimetre of flapper travel spans
the whole working range of $P_{b}$ --- which is exactly what makes the device
useful as a sensor: it turns a mechanical signal too small to measure any
other way into a pressure signal that is easy to measure and to act on.

% --------------------------------------------------------------------
\subsection{The pneumatic relay}
\label{sss:relaygain}

Keeping the flapper motion inside the linear region of
Figure~\ref{fig:flapper} also keeps the output pressure swing small, so a
pneumatic \emph{power amplifier} --- the \textbf{relay} --- is cascaded after
it. Figure~\ref{fig:relay} shows the idea: the back pressure $P_{b}$ acts on
a small bellows, which positions a ball between two seats. Seated on the
\emph{upper} seat, the ball blocks the vent to atmosphere, so supply air
fills the output line and the output pressure $P$ rises to $P_{s}$; seated on
the \emph{lower} seat, the ball blocks the supply, so the output vents to
atmosphere and $P$ falls towards zero. Moving the flapper \emph{away} from the
nozzle in Figure~\ref{fig:flapper} \emph{drops} $P_{b}$ (less restriction, air
escapes more freely); the relay bellows then contracts, the ball rises off
the lower seat towards the upper one, the vent path closes, and $P$
\emph{rises}. The relay therefore inverts sign as well as amplifying: raising
$x$ lowers $P_{b}$ (\eqref{eq:flapper}, $K<0$) but raises $P$, so overall
\begin{equation}
  \frac{\Delta P(s)}{\Delta X(s)}=\left(\frac{a}{a+b}\right)K, \qquad K>0.
  \label{eq:relay}
\end{equation}
Nothing here needs a new constant: the relay's job is to reproduce
$P_{b}$'s \emph{shape} at supply-line power, and the two sign flips ---
flapper-to-$P_b$ negative, relay negative again --- cancel, so \eqref{eq:relay}
uses the same magnitude $|K|$ and lever ratio as \eqref{eq:flapper}.

\begin{figure}[H]
\centering
\begin{tikzpicture}[font=\small, x=1.1cm, y=1.1cm]
% ---- relay bellows, driven by Pb ----
\draw[thick] (0,0) rectangle (1.7,1.5);
\node[font=\scriptsize, align=center] at (0.85,0.75) {relay\\bellows};
\draw[sig, faccent] (0.85,-0.05) -- (0.85,-0.75);
\node[faccent, font=\scriptsize, below] at (0.85,-0.80) {$P_b$ in};
% ---- pushrod up to the ball ----
\draw[very thick] (0.85,1.5) -- (0.85,2.15);
\draw[fill] (0.85,2.35) circle (5.5pt);
% ---- lower seat (supply, below the ball) and upper seat (vent, above) ----
\draw[thick] (0.50,2.05) -- (0.85,2.25);
\draw[thick] (1.20,2.05) -- (0.85,2.25);
\draw[thick] (0.50,2.65) -- (0.85,2.45);
\draw[thick] (1.20,2.65) -- (0.85,2.45);
% ---- supply (into the lower seat), vent (up from the upper seat), output ----
\draw[sig, faccent] (-0.85,2.10) -- (0.45,2.10);
\node[faccent, font=\scriptsize, left, align=right] at (-0.90,2.10)
   {supply\\$P_s$};
\node[font=\scriptsize, anchor=west] at (1.35,1.95) {(supply seat)};
\draw[sig] (0.85,2.65) -- (0.85,3.45);
\node[font=\scriptsize, above, align=center] at (0.85,3.5) {vent\\(vent seat)};
\draw[sig, fnavy] (1.25,2.35) -- (2.6,2.35);
\node[fnavy, font=\scriptsize, right, align=left] at (2.65,2.35)
   {output\\$P$ (to diaphragm)};
\end{tikzpicture}
\caption{Pneumatic relay (power amplifier), schematic. The flapper's back
pressure $P_b$ positions a lightweight ball between a supply seat and a vent
seat; whichever seat it leaves open decides whether the output line fills
from supply or empties to atmosphere. Output pressure swings over the full
supply range for a very small ball travel, and the sign is inverted relative
to $P_b$.}
\label{fig:relay}
\end{figure}
...
Most pneumatic control systems need a translational output, and this is the
device that supplies it. A diaphragm of area $A$, Figure~\ref{fig:pneuact},
is exposed to the controlled pressure $P$ and moves a stem against a
\textbf{return spring} of stiffness $K$; the load contributes mass $M$ and
friction $f$. Unlike the bellows of \S\ref{sss:bellows}, here the moving mass
is large enough, and the motion fast enough, that inertia cannot be dropped
...
\end{tikzpicture}
\caption{Pneumatic diaphragm actuator. Unlike the hydraulic cylinder, the
spring gives it a definite equilibrium position for each pressure --- so its
transfer function \eqref{eq:pneuact} is second order with \emph{no} free
integrator. Cf.\ Nagrath \& Gopal Fig.~4.37, p.~119.}
\label{fig:pneuact}
\end{figure}

\begin{pitfall}
Compare \eqref{eq:pneuact} directly with the hydraulic cylinder,
\eqref{eq:cyl}. The cylinder has a free $1/s$; the spring-loaded diaphragm
does not. The difference is exactly one part, the spring, but its
consequence is not small: with a spring present, a \emph{constant} pressure
gives a \emph{constant position} (the spring force balances it at a definite
displacement); without one, a constant flow gives a constant \emph{velocity},
and position keeps growing for as long as the pressure is applied. Read the
physics, not the picture --- both figures show a piston in a cylinder pushed
by a fluid, and yet one integrates and the other does not.
\end{pitfall}

\begin{workedex}[breakable, title={Worked example 5.1 --- sizing a diaphragm actuator's response}]
A diaphragm actuator has area $A=\SI{0.01}{\metre\squared}$, spring stiffness
$K=\SI{40000}{\newton\per\metre}$, moving mass $M=\SI{1}{\kilo\gram}$ and
friction $f=\SI{240}{\newton\second\per\metre}$, driving a pneumatic control
valve. Write \eqref{eq:pneuact} in standard second-order form,
\[
  \frac{\Delta Y(s)}{\Delta P(s)}=\frac{A/K}{s^{2}/\wn^{2}+2\zt s/\wn+1},
  \qquad
  \wn=\sqrt{\frac{K}{M}}, \qquad \zt=\frac{f}{2\sqrt{KM}}.
\]
Numerically, $\wn=\sqrt{40000/1}=\SI{200}{\radian\per\second}$ and
$\zt=240/(2\sqrt{40000})=0.60$: underdamped, as a control-valve actuator with
some mechanical damping typically is. The steady-state gain is
$A/K=\SI{2.5e-7}{\metre\per\pascal}$, so a \SI{40}{\kilo\pascal} step in
controlled pressure --- a normal swing for a process instrument --- drives the
stem $\Delta y_{ss}=A/K\times\SI{40000}{\pascal}=\SI{10}{\milli\metre}$ in
steady state. From $\zt=0.6$, the standard second-order formulas (Week~5)
give
\[
  \%\text{OS}=100\,e^{-\pi\zt/\sqrt{1-\zt^{2}}}=9.5\%,
  \qquad
  t_{s}(2\%)\approx\frac{4}{\zt\wn}=\SI{33}{\milli\second}.
\]
Figure~\ref{fig:diaphragmstep} shows the response. The whole transient is
over in about \SI{50}{\milli\second} --- consistent with \S\ref{sss:cylinder}'s
observation that the actuator's own dynamics are usually the fast part of a
pneumatic loop; it is the \emph{lines} carrying the pressure signal to and
from it, not modelled here, that are pneumatics' real speed limit.
...
\begin{axis}[
  ...
  xlabel={time (ms)}, ylabel={$\Delta y/\Delta y_{ss}$},
  xlabel style={font=\scriptsize}, ylabel style={font=\scriptsize},
  domain=0:50, samples=300,
  xmin=0, xmax=50, ymin=0, ymax=1.22,
  ytick={0,0.5,1}, tick label style={font=\scriptsize},
  grid=both, grid style={gray!18},
  axis lines=left, clip=false,
]
\addplot[thin, gray, dashed, forget plot, domain=0:50, samples=2] {1};
\addplot[thick, fnavy]
  {1-exp(-0.12*x)*(cos(deg(0.16*x))+0.75*sin(deg(0.16*x)))};
\node[fnavy, font=\scriptsize, anchor=south] at (axis cs:19.6,1.10) {9.5\% overshoot};
\draw[gray, thin, -{Latex[length=1.4mm]}] (axis cs:33.3,0.20) -- (axis cs:33.3,0.98);
\node[gray, font=\scriptsize, anchor=north] at (axis cs:33.3,0.19) {$t_{s}\approx\SI{33}{ms}$};
\end{axis}
\end{tikzpicture}
\caption{Step response of the diaphragm actuator of Worked example 5.1:
$\wn=\SI{200}{rad/s}$, $\zt=0.60$. Compare the shape with
Figure~\ref{fig:ratefb} --- same mathematics, a mechanical spring supplying
the restoring force instead of a synchro-and-amplifier loop.}
\label{fig:diaphragmstep}
\end{figure}
\end{workedex}

% --------------------------------------------------------------------
\subsection{Comparison: electric, hydraulic, pneumatic}
\label{sss:comparison}
...
\begin{keyidea}
Do not memorise the table row by row; reason with the three trade-offs behind
it. \textbf{Power density} pushes a designer towards hydraulics,
\textbf{safety and cost} towards pneumatics, \textbf{precision and
convenience} towards electrics. Almost every real actuator selection in
practice is one of those three arguments winning over the other two.
\end{keyidea}

% ====================================================================
\subsection*{Summary of \S\S\ref{sec:hydraulics}--\ref{sec:pneumatics}}

\begin{keyidea}
Each equation in \S\S\ref{sec:hydraulics}--\ref{sec:pneumatics}, and where it
came from:
\begin{center}
\begin{tabular}{clP{6.5cm}}
\toprule
Eq. & Result & Origin \\
\midrule
\eqref{eq:hyd} & $K/[s(\tau s+1)]$
   & flow balance and torque balance, $p$ eliminated \\
--- & $K_{T}=K_{m}$
   & power balance $p\,q_{m}=T_{m}\dot\theta$, exactly as $K_{t}=K_{b}$ \\
\eqref{eq:valve} & $q=K_{1}x-K_{2}p$
   & linearisation of $q\propto\sqrt{\Delta p}$ about neutral \\
\eqref{eq:cyl} & $K/[s(\tau s+1)]$, $K=AK_{1}/(A^{2}+K_{2}f)$
   & valve relation, continuity $q=A\dot y$, force balance $Ap=M\ddot y+f\dot y$ \\
\eqref{eq:bellows} & $A/K$, pure gain
   & spring balance $(\Delta P)A=K\Delta x$ \\
\eqref{eq:flapper} & $(a/(a+b))K$, $K<0$
   & lever arm $\times$ local slope of the $P_b$--$e$ curve \\
\eqref{eq:relay} & $(a/(a+b))K$, $K>0$
   & same magnitude, sign flips again in the ball-and-seat linkage \\
\eqref{eq:pneuact} & $A/(Ms^{2}+fs+K)$
   & force balance with a spring: no free integrator \\
\bottomrule
\end{tabular}
\end{center}
\end{keyidea}
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
\item Why is $K_{T}=K_{m}$ for a hydraulic motor for exactly the same reason
      $K_{t}=K_{b}$ for a d.c.\ motor? Name the one physical law both
      arguments use.
\item \eqref{eq:hyd} was derived after dropping the compressibility flow
      $K_{c}\dot p$. What term would that add back into the model, and what
      order would the transfer function become?
\item In Worked example 4.1, the leakage term $K_{2}f$ was 5\% of $A^{2}$.
      Without recomputing anything, say which way $K$ and $\tau$ would move
      if a worn seal doubled $K_{2}$, and which of the two errors --- in $K$
      or in $\tau$ --- would grow faster.
\item The hydraulic cylinder \eqref{eq:cyl} has a free integrator; the
      pneumatic diaphragm actuator \eqref{eq:pneuact} does not, even though
      both are ``a fluid pushing a piston''. What single component makes the
      difference, and why?
\item The flapper--nozzle gain \eqref{eq:flapper} is negative and the relay
      gain \eqref{eq:relay} is positive, yet the text says both use ``the
      same magnitude $|K|$''. Reconcile these two statements.
\item In Worked example 5.1, doubling the return spring stiffness $K$ (with
      $M$ and $f$ unchanged) changes $\wn$, $\zt$ and the steady-state gain.
      State the direction of each change without recomputing the numbers.
\end{enumerate}
